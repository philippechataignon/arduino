/*
 * Clignotement d'une DEL (LED en anglais) Ã  1 Hz par timer 2.
 */
...
void setup ()
{
  ...
  TCNT2 = 256 - 250;        // Chargement du timer Ã  6
}
...
void loop () {
  if (bitRead (TIFR2, 0) == 1) {       // Flag TOV2 mis Ã  1 ?
    TCNT2 = 256 - 250;         // Rechargement du timer Ã  6
    bitSet (TIFR2, TOV2);      // Remise Ã  zÃ©ro du flag TOV2 (voir texte)
    if (varCompteur++ > 125) { // IncrÃ©mentation et a atteint 125 ?
